...
# --------------------------------------------------------------------------
# Inline SVG icon set (no external requests, no CDN; colours come from the
# light/dark tokens below)
# --------------------------------------------------------------------------
_SUN_FULL = (
    '<circle cx="12" cy="12" r="4.6" fill="var(--w-sun)"/>'
    '<g stroke="var(--w-sun)" stroke-width="1.7" stroke-linecap="round">'
...
_MOON_FULL = '<path d="M20.5 14.8A8.6 8.6 0 0 1 9.2 3.5 8.6 8.6 0 1 0 20.5 14.8z" fill="var(--w-moon)"/>'
_SUN_SMALL = (
    '<circle cx="8.4" cy="7.4" r="3.3" fill="var(--w-sun)"/>'
    '<g stroke="var(--w-sun)" stroke-width="1.5" stroke-linecap="round">'
    '<path d="M8.4 1.6v1.6"/><path d="M2.6 7.4H1"/>'
    '<path d="M4.1 3.1L3 2"/><path d="M4.1 11.7L3 12.8"/></g>'
)
_MOON_SMALL = (
    '<path d="M12.4 8.6A5.2 5.2 0 0 1 5.6 1.8 5.2 5.2 0 1 0 12.4 8.6z" fill="var(--w-moon)"/>'
)
_CLOUD_LOW = '<path d="M17.8 19.2H6.4A4.4 4.4 0 0 1 6 10.4 6.3 6.3 0 0 1 17.6 9.6 4.8 4.8 0 0 1 17.8 19.2z" fill="var(--w-cloud)"/>'
_CLOUD_HIGH = '<path d="M17.4 16.2H6.2A4.1 4.1 0 0 1 5.8 8.1 6 6 0 0 1 17.1 7.4 4.5 4.5 0 0 1 17.4 16.2z" fill="var(--w-cloud)"/>'
_CLOUD_SIDE = '<path d="M18.6 19.4H8.2A4.1 4.1 0 0 1 7.9 11.3 6 6 0 0 1 18.4 10.6 4.4 4.4 0 0 1 18.6 19.4z" fill="var(--w-cloud)"/>'
...
_ICONS = {
    ...
    + '<g stroke="var(--w-fog)" stroke-width="1.7" stroke-linecap="round">'
    '<path d="M4.8 19.2h14.4"/><path d="M7.2 22.2h10.6"/></g>',
    "drizzle": lambda day: _CLOUD_HIGH + _drops("var(--w-rain)", 3, False),
    "rain": lambda day: _CLOUD_HIGH + _drops("var(--w-rain-strong)", 3, True),
    "showers": lambda day: _CLOUD_HIGH + _drops("var(--w-rain)", 2, True),
    "sleet": lambda day: _CLOUD_HIGH
    + _drops("var(--w-rain)", 2, False)
    + '<circle cx="16.4" cy="20.4" r="1.2" fill="var(--w-snow)"/>',
    "snow": lambda day: _CLOUD_HIGH
    + '<g fill="var(--w-snow)"><circle cx="8" cy="19.6" r="1.3"/>'
    '<circle cx="12" cy="21.8" r="1.3"/><circle cx="16" cy="19.6" r="1.3"/></g>',
    "thunder": lambda day: _CLOUD_HIGH
    + '<path d="M13.4 16.6l-4.2 5.3h3L11 24.2l4.6-5.6h-3.1l1.9-2z" fill="var(--w-sun)"/>',
}
...
_DARK_TOKENS = """
  color-scheme: dark;
  --w-card-bg: rgba(20,20,25,0.4);
  --w-card-border: rgba(255,255,255,0.08);
  --w-card-shadow: rgba(0,0,0,0.3);
  --w-surface: rgba(255,255,255,0.03);
  --w-surface-border: rgba(255,255,255,0.05);
  --w-surface-border-strong: rgba(255,255,255,0.06);
  --w-divider: rgba(255,255,255,0.04);
  --w-text: #f0f0f0;
  --w-text-2: #cccccc;
  --w-text-3: #aaaaaa;
  --w-muted-strong: #999999;
  --w-muted: #888888;
  --w-faint: #666666;
  --w-label: #555555;
  --w-attribution: #444444;
  --w-accent: #6cb4ee;
  --w-tab-bg: rgba(255,255,255,0.04);
  --w-tab-border: rgba(255,255,255,0.08);
  --w-tab-fg: #888888;
  --w-tab-active-bg: rgba(255,255,255,0.12);
  --w-tab-active-border: rgba(255,255,255,0.15);
  --w-tab-active-fg: #ffffff;
  --w-scroll-track: rgba(255,255,255,0.05);
  --w-scroll-thumb: rgba(255,255,255,0.2);
  --w-scroll-thumb-hover: rgba(255,255,255,0.3);
  --w-sun: #fbbf24;
  --w-moon: #e2e8f0;
  --w-cloud: #cbd5e1;
  --w-fog: #94a3b8;
  --w-rain: #60a5fa;
  --w-rain-strong: #3b82f6;
  --w-snow: #e0f2fe;
  --w-sunrise: #f59e0b;
  --w-sunset: #8b5cf6;
"""

# Light values are emitted twice (media query + explicit data-theme), so keep
# them in one place.
_LIGHT_TOKENS = """
  color-scheme: light;
  --w-card-bg: rgba(255,255,255,0.72);
  --w-card-border: rgba(0,0,0,0.10);
  --w-card-shadow: rgba(0,0,0,0.10);
  --w-surface: rgba(0,0,0,0.035);
  --w-surface-border: rgba(0,0,0,0.08);
  --w-surface-border-strong: rgba(0,0,0,0.10);
  --w-divider: rgba(0,0,0,0.07);
  --w-text: #17171b;
  --w-text-2: #3f3f46;
  --w-text-3: #52525b;
  --w-muted-strong: #4b5563;
  --w-muted: #5a6270;
  --w-faint: #6b7280;
  --w-label: #6b7280;
  --w-attribution: #767e8c;
  --w-accent: #1b74c4;
  --w-tab-bg: rgba(0,0,0,0.04);
  --w-tab-border: rgba(0,0,0,0.10);
  --w-tab-fg: #5a6270;
  --w-tab-active-bg: rgba(0,0,0,0.10);
  --w-tab-active-border: rgba(0,0,0,0.16);
  --w-tab-active-fg: #111114;
  --w-scroll-track: rgba(0,0,0,0.06);
  --w-scroll-thumb: rgba(0,0,0,0.22);
  --w-scroll-thumb-hover: rgba(0,0,0,0.34);
  --w-sun: #f59e0b;
  --w-moon: #8593a8;
  --w-cloud: #93a3b8;
  --w-fog: #7c8899;
  --w-rain: #3b82f6;
  --w-rain-strong: #2563eb;
  --w-snow: #7fb8dd;
  --w-sunrise: #d97706;
  --w-sunset: #7c3aed;
"""

# Dark stays the default; light wins when the OS asks for it, and the JS below
# can pin either one when it can see the host app's theme.
_WIDGET_CSS = (
    """
<style>
:root {"""
...
#weather___WID__ .hourly-strip::-webkit-scrollbar-track { background: var(--w-scroll-track); border-radius: 2px; }
#weather___WID__ .hourly-strip::-webkit-scrollbar-thumb { background: var(--w-scroll-thumb); border-radius: 2px; }
#weather___WID__ .hourly-strip::-webkit-scrollbar-thumb:hover { background: var(--w-scroll-thumb-hover); }
</style>
"""
)
...
(function() {
    ...
    // Open WebUI's own theme toggle can disagree with the OS setting this
    // iframe sees, so mirror the host's theme when the sandbox lets us read
    // it. Cross-origin sandboxes throw here; CSS prefers-color-scheme covers
    // that case on its own.
    try {
        var host = window.parent.document.documentElement;
        var applyTheme = function() {
            var theme = host.getAttribute('data-theme') || '';
            var dark = host.classList.contains('dark') || theme.indexOf('dark') !== -1;
            document.documentElement.setAttribute('data-theme', dark ? 'dark' : 'light');
        };
        applyTheme();
        if (typeof MutationObserver !== 'undefined') {
            new MutationObserver(applyTheme).observe(host, {
                attributes: true,
                attributeFilter: ['class', 'data-theme']
            });
        }
    } catch (e) { /* sandboxed: fall back to prefers-color-scheme */ }
        ...
        tabs.forEach(function(tab) {
            ...
            tab.btn.addEventListener('click', function() {
                tabs.forEach(function(t) {
                    t.btn.style.background = 'var(--w-tab-bg)';
                    t.btn.style.borderColor = 'var(--w-tab-border)';
                    t.btn.style.color = 'var(--w-tab-fg)';
                    t.panel.style.display = 'none';
                });
                tab.btn.style.background = 'var(--w-tab-active-bg)';
                tab.btn.style.borderColor = 'var(--w-tab-active-border)';
                tab.btn.style.color = 'var(--w-tab-active-fg)';
...
def _generate_weather_embed(
    ...
    for h in hourly_items:
        ...
        hourly_temp_html += f"""
        <div style="{cell}">
            <span style="font-size:10px;color:var(--w-muted);font-weight:500;">{_esc(h["time"])}</span>
            {icon}
            <span style="font-size:11px;color:var(--w-muted-strong);">{h["pop"]}%</span>
            <span style="font-size:14px;color:var(--w-text);font-weight:600;">{h["temp"]}{temp_unit}</span>
        </div>"""
        ...
        <div style="{cell}">
            <span style="font-size:10px;color:var(--w-muted);font-weight:500;">{_esc(h["time"])}</span>
            {icon}
            <span style="font-size:14px;color:var(--w-accent);font-weight:600;">{h["pop"]}%</span>
            <span style="font-size:11px;color:var(--w-muted-strong);">{h["temp"]}{temp_unit}</span>
        </div>"""

        hourly_wind_html += f"""
        <div style="{cell}">
            <span style="font-size:10px;color:var(--w-muted);font-weight:500;">{_esc(h["time"])}</span>
            <svg viewBox="0 0 24 24" style="width:20px;height:20px;fill:var(--w-text-3);transform:rotate({h["wind_deg"]}deg);"><path d="M12 2L4.5 20.3l.7.7L12 18l6.8 3 .7-.7z"/></svg>
            <span style="font-size:14px;color:var(--w-text);font-weight:600;">{h["wind"]}</span>
            <span style="font-size:10px;color:var(--w-muted);">{wind_unit}</span>
        </div>"""
    ...
    for d in daily_items:
        ...
        daily_cards_html += f"""
        <div style="background:var(--w-surface);border:1px solid var(--w-surface-border-strong);border-radius:10px;padding:12px;display:flex;flex-direction:column;align-items:center;gap:4px;min-width:0;">
            <div style="font-size:13px;color:var(--w-text);font-weight:600;">{_esc(d["day"])}</div>
            <div style="font-size:10px;color:var(--w-faint);">{_esc(d["date"])}</div>
            {icon}
            <div style="font-size:11px;color:var(--w-muted-strong);text-transform:capitalize;text-align:center;line-height:1.3;">{_esc(d["desc"])}</div>
            <div style="margin-top:auto;padding-top:4px;">
                <span style="font-size:16px;color:var(--w-text);font-weight:600;">{d["high"]}°</span>
                <span style="font-size:12px;color:var(--w-faint);margin-left:2px;">{d["low"]}°</span>
            </div>
            <div style="display:flex;gap:6px;">
                <span style="font-size:10px;color:var(--w-accent);">💧{d["pop"]}%</span>
                <span style="font-size:10px;color:var(--w-muted);">🌬{d["wind"]}</span>
            </div>
        </div>"""

        daily_list_html += f"""
        <div style="display:flex;align-items:center;padding:8px 0;border-bottom:1px solid var(--w-divider);gap:8px;">
            <div style="min-width:50px;">
                <div style="font-size:13px;color:var(--w-text);font-weight:600;">{_esc(d["day"])}</div>
                <div style="font-size:10px;color:var(--w-faint);">{_esc(d["date"])}</div>
            </div>
            {_icon_svg(d["code"], True, 30)}
            <div style="flex:1;font-size:11px;color:var(--w-muted-strong);text-transform:capitalize;">{_esc(d["desc"])}</div>
            <div style="display:flex;align-items:center;gap:6px;">
                <span style="font-size:10px;color:var(--w-accent);">💧{d["pop"]}%</span>
                <span style="font-size:10px;color:var(--w-muted);">🌬{d["wind"]}</span>
            </div>
            <div style="min-width:65px;text-align:right;">
                <span style="font-size:14px;color:var(--w-text);font-weight:600;">{d["high"]}°</span>
                <span style="font-size:12px;color:var(--w-faint);margin-left:4px;">{d["low"]}°</span>
            </div>
        </div>"""

    tile = (
        "background:var(--w-surface);border:1px solid var(--w-surface-border);"
        "border-radius:8px;padding:10px;text-align:center;"
    )
    label = (
        "font-size:8px;color:var(--w-label);text-transform:uppercase;letter-spacing:0.5px;"
        "margin-bottom:4px;font-weight:700;"
    )
    value = "font-size:16px;color:var(--w-text);font-weight:500;"
    ...
    if hourly_items:
        ...
        <div style="margin-bottom:16px;">
            <div style="display:flex;gap:8px;margin-bottom:12px;flex-wrap:wrap;">
                <button id="tabTemp_{widget_id}" style="background:var(--w-tab-active-bg);border:1px solid var(--w-tab-active-border);color:var(--w-tab-active-fg);padding:6px 14px;border-radius:20px;font-size:11px;cursor:pointer;transition:all 0.2s;font-family:inherit;">Temperature</button>
                <button id="tabPrecip_{widget_id}" style="background:var(--w-tab-bg);border:1px solid var(--w-tab-border);color:var(--w-tab-fg);padding:6px 14px;border-radius:20px;font-size:11px;cursor:pointer;transition:all 0.2s;font-family:inherit;">Precipitation</button>
                <button id="tabWind_{widget_id}" style="background:var(--w-tab-bg);border:1px solid var(--w-tab-border);color:var(--w-tab-fg);padding:6px 14px;border-radius:20px;font-size:11px;cursor:pointer;transition:all 0.2s;font-family:inherit;">Wind</button>
            </div>
            <div id="hourlyTemp_{widget_id}" class="hourly-strip" style="display:flex;gap:4px;overflow-x:auto;padding-bottom:6px;scrollbar-width:thin;scrollbar-color:var(--w-scroll-thumb) var(--w-scroll-track);">
                {hourly_temp_html}
            </div>
    ...
    if daily_items:
        daily_section = f"""
        <div style="border-top:1px solid var(--w-surface-border);padding-top:12px;">
            <div style="font-size:8px;font-weight:700;color:var(--w-label);text-transform:uppercase;letter-spacing:0.5px;margin-bottom:8px;">Forecast</div>
    ...
    gust_line = ""
    if cur_gust:
        gust_line = f'<div style="font-size:9px;color:var(--w-faint);margin-top:2px;">{wind_dir_str} · gust {cur_gust}</div>'
    else:
        gust_line = f'<div style="font-size:9px;color:var(--w-faint);margin-top:2px;">{wind_dir_str}</div>'
...
<body>
    <div style="display:flex;justify-content:center;width:100%;">
      <div id="weather_{widget_id}" style="background:var(--w-card-bg);backdrop-filter:blur(10px);border:1px solid var(--w-card-border);border-radius:12px;padding:20px 24px;box-shadow:0 8px 32px var(--w-card-shadow);max-width:800px;width:100%;font-family:system-ui,-apple-system,sans-serif;">

        <div style="display:flex;align-items:flex-start;justify-content:space-between;margin-bottom:16px;gap:12px;">
            <div>
                <div style="font-size:18px;font-weight:600;color:var(--w-text);letter-spacing:-0.2px;">{_esc(location_name)}</div>
                <div style="font-size:10px;color:var(--w-muted);font-weight:500;text-transform:uppercase;letter-spacing:1px;margin-top:2px;">{subtitle}</div>
            </div>
            <div style="font-size:20px;color:var(--w-text-3);font-weight:300;text-align:right;">{_esc(time_str)}</div>
        </div>

        <div style="display:flex;align-items:center;gap:14px;margin-bottom:20px;">
            {_icon_svg(cur_code, cur_is_day, 68)}
            <div>
                <div style="font-size:42px;font-weight:300;color:var(--w-text);line-height:1;letter-spacing:-2px;">{cur_temp}<span style="font-size:20px;color:var(--w-muted);font-weight:400;">{temp_unit}</span></div>
                <div style="font-size:14px;color:var(--w-text-2);margin-top:2px;">{_esc(cur_desc)}</div>
            </div>
        </div>
        ...
        <div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(110px,1fr));gap:8px;margin-bottom:20px;">
            ...
            <div style="{tile}">
                ...
                <div style="{value}">{cur_wind} <span style="font-size:10px;color:var(--w-muted);">{wind_unit}</span></div>
                {gust_line}
            </div>
            <div style="{tile}">
                <div style="{label}">Pressure</div>
                <div style="{value}">{cur_pressure} <span style="font-size:10px;color:var(--w-muted);">hPa</span></div>
            </div>
            <div style="{tile}">
            ...
            <div style="{tile}">
                ...
                <div style="{value}">{vis_str} <span style="font-size:10px;color:var(--w-muted);">km</span></div>
            </div>
        </div>

        <div style="display:flex;gap:12px;margin-bottom:20px;">
            <div style="flex:1;background:var(--w-surface);border:1px solid var(--w-surface-border);border-radius:8px;padding:10px;display:flex;align-items:center;gap:8px;">
                <svg viewBox="0 0 24 24" style="width:18px;height:18px;fill:var(--w-sunrise);flex-shrink:0;"><path d="M12 7a5 5 0 1 0 0 10 5 5 0 0 0 0-10zm0-3a1 1 0 0 0 1-1V1a1 1 0 0 0-2 0v2a1 1 0 0 0 1 1zm0 18a1 1 0 0 0-1 1v2a1 1 0 0 0 2 0v-2a1 1 0 0 0-1-1zM5.64 5.64a1 1 0 0 0 0-1.41l-1.42-1.42a1 1 0 1 0-1.41 1.42l1.42 1.41a1 1 0 0 0 1.41 0zM19.78 18.36a1 1 0 1 0-1.41 1.42l1.42 1.41a1 1 0 0 0 1.41-1.41l-1.42-1.42zM4 12a1 1 0 0 0-1-1H1a1 1 0 0 0 0 2h2a1 1 0 0 0 1-1zm19-1h-2a1 1 0 0 0 0 2h2a1 1 0 0 0 0-2zM5.64 18.36l-1.42 1.42a1 1 0 0 0 1.41 1.41l1.42-1.41a1 1 0 0 0-1.41-1.42zM19.78 5.64a1 1 0 0 0 .7-.29l1.42-1.42a1 1 0 1 0-1.41-1.41l-1.42 1.41a1 1 0 0 0 .71 1.71z"/></svg>
                <div>
                    <div style="{label}">Sunrise</div>
                    <div style="font-size:14px;color:var(--w-text);font-weight:500;">{sunrise_str}</div>
                </div>
            </div>
            <div style="flex:1;background:var(--w-surface);border:1px solid var(--w-surface-border);border-radius:8px;padding:10px;display:flex;align-items:center;gap:8px;">
                <svg viewBox="0 0 24 24" style="width:18px;height:18px;fill:var(--w-sunset);flex-shrink:0;"><path d="M12 17a5 5 0 1 0 0-10 5 5 0 0 0 0 10zm0 1a1 1 0 0 0-1 1v2a1 1 0 0 0 2 0v-2a1 1 0 0 0-1-1zm7.78-.64l-1.42-1.42a1 1 0 1 0-1.41 1.42l1.42 1.41a1 1 0 0 0 1.41-1.41zM20 12a1 1 0 0 0 1 1h2a1 1 0 0 0 0-2h-2a1 1 0 0 0-1 1zM5.64 5.64a1 1 0 0 0 0-1.41L4.22 2.81a1 1 0 1 0-1.41 1.42l1.42 1.41a1 1 0 0 0 1.41 0zM4 12a1 1 0 0 0-1-1H1a1 1 0 0 0 0 2h2a1 1 0 0 0 1-1zm1.64 6.36l-1.42 1.42a1 1 0 0 0 1.41 1.41l1.42-1.41a1 1 0 0 0-1.41-1.42zM12 7a1 1 0 0 0 1-1V4a1 1 0 0 0-2 0v2a1 1 0 0 0 1 1zm7.78-4.19l-1.42 1.41a1 1 0 1 0 1.41 1.42l1.42-1.42a1 1 0 0 0-1.41-1.41z"/></svg>
                <div>
                    <div style="{label}">Sunset</div>
                    <div style="font-size:14px;color:var(--w-text);font-weight:500;">{sunset_str}</div>
                </div>
            </div>
        </div>
        ...
        <div style="margin-top:12px;text-align:center;">
            <span style="font-size:9px;color:var(--w-attribution);">Weather data by Open-Meteo.com (CC BY 4.0)</span>
        </div>
      </div>
    </div>
{js}
